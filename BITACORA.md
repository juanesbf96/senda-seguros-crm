...
> **Ítem 3 — resuelto (7-ago, PR #45, Carril A).** Resultó **más simple** de lo temido: **no** necesitó tocar `notificaciones_renovacion` ni migración. El dedup se hace contra la **propia tabla `operaciones`** por `(poliza_id, fecha_programada)` — una renovación por póliza por ciclo; al renovar, la `fecha_fin` avanza y el próximo ciclo genera otra. El cron amplió su ventana a **60 días** para crear la operación temprano (los emails no cambian: siguen filtrando ≤30). Operación: `tipo='renovacion'`, `estado_cartera='pendiente'`, `valor=prima_neta`, `origen='cron_renovaciones'`. Mismo patrón app-level que el dedup de emails (el cron corre secuencial). Probado en staging: 2 corridas no duplican, un ciclo nuevo crea la 2ª.

> **Alerta de certificados (fase 2.4) — RESUELTA (8-ago, PR #55, Carril A).** El mismo cron ahora avisa por certificados que vencen en 30/15/7 días, además de las pólizas. El blocker era la `UNIQUE (poliza_id, dias_alerta, fecha_envio)` de `notificaciones_renovacion`, que no dejaba registrar dos certificados de la misma póliza: se reemplazó por un **índice único sobre `COALESCE(certificado_id, poliza_id)`** (las pólizas siguen deduplicando por `poliza_id`; los certificados por `certificado_id`) + columnas `tipo` y `certificado_id` + RPC `get_certificados_por_vencer`. La lógica del cron es **paralela** a la de pólizas (no toca ese flujo); el email suma una sección "Certificados por vencer". Probado en staging (RPC filtra; 2 certs misma póliza/día se registran; el índice bloquea el mismo cert; dedup de póliza intacto). Migración aplicada en prod (SQL Editor, 8-ago). **0 certificados en prod → riesgo nulo.**
