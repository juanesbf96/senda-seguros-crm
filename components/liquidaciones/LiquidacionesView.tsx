  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-ink-700">Liquidaciones</h1>
          <p className="text-ink-400 text-sm mt-1">Comisiones por vendedor y período</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-5">
        <div className="bg-white border border-warning/30 rounded-xl p-4">
          <p className="text-xs text-ink-700 font-medium mb-1">Por pagar</p>
          <p className="text-xl font-bold text-ink-700">{formatCOP(totalPendiente)}</p>
          <p className="text-xs text-ink-400 mt-0.5">{filtered.filter(l => l.estado === 'pendiente').length} liquidaciones</p>
        </div>
        <div className="bg-white border border-primary-200 rounded-xl p-4">
          ...
          <p className="text-xl font-bold text-primary-700">{formatCOP(totalPagado)}</p>
          <p className="text-xs text-ink-400 mt-0.5">{filtered.filter(l => l.estado === 'pagado').length} liquidaciones</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-ink-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map(l => (
              <tr key={l.id} className="border-b border-cream-200 hover:bg-cream-100 transition-colors">
                <td className="px-4 py-3 font-medium text-ink-700">{l.vendedor?.nombre || '—'}</td>
                <td className="px-4 py-3 text-ink-500 font-mono text-xs">{l.periodo}</td>
                <td className="px-4 py-3 text-right text-ink-500">{formatCOP(l.total_primas)}</td>
                <td className="px-4 py-3 text-right font-semibold text-ink-700">{formatCOP(l.total_comision)}</td>
                <td className="px-4 py-3 hidden md:table-cell text-ink-400 text-xs">{formatDate(l.fecha_pago)}</td>
