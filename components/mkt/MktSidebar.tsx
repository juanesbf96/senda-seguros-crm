...
export default function MktSidebar() {
  ...
  if (!mounted) {
    return <aside className="w-60 flex-shrink-0 bg-ink-700" />
  }

  return (
    <aside className={[
      'flex-shrink-0 bg-ink-700 text-white flex flex-col h-full',
      'transition-[width] motion-reduce:transition-none duration-[220ms] ease-out',
      collapsed ? 'w-[60px]' : 'w-60',
    ].join(' ')}>

      {/* ── Header ── */}
      <div className="h-[60px] flex-shrink-0 border-b border-ink-600 flex items-center px-2.5 gap-2 overflow-hidden">
        {!collapsed && (
          <>
        ...
        <button onClick={toggle}
          ...
          className={[
            ...
            'text-sky-400 hover:text-white hover:bg-ink-700 active:bg-ink-600',
            ...
            <div key={group.id} className="mb-1">
              {!collapsed && (
                <button onClick={() => toggleGroup(group.id)}
                  className="w-full flex items-center justify-between px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-ink-400 hover:text-ink-300 transition-colors">
                    ...
                    return (
                      <div key={href} className="relative group">
                        <Link href={soon ? '#' : href}
                          ...
                          {!collapsed && soon && (
                            <span className="text-[9px] font-bold bg-ink-600 text-ink-300 px-1.5 py-0.5 rounded uppercase tracking-wide">
                              Soon
                            </span>
                          )}
                        </Link>
                        {/* Tooltip */}
                        {collapsed && (
                          <div role="tooltip" className="pointer-events-none absolute left-full top-1/2 -translate-y-1/2 ml-3 z-50 bg-ink-700 border border-ink-500 text-white text-xs font-medium px-2.5 py-1.5 rounded-lg shadow-xl whitespace-nowrap opacity-0 translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-[opacity,transform] duration-150 ease-out">
              ...
              {collapsed && <div className="mx-3 my-1 border-t border-ink-600/40" />}
            </div>
          )
        })}
      </nav>
      ...
      {/* ── Footer ── */}
      <div className="border-t border-ink-600 px-2 py-2 space-y-0.5">
        <div className="relative group">
          <Link href="/configuracion"
            className={[
              'flex items-center rounded-lg text-sm font-medium min-h-[38px] transition-colors duration-150',
              collapsed ? 'justify-center px-2' : 'gap-3 px-3',
              'text-sky-400 hover:bg-ink-700 hover:text-white',
            ].join(' ')}>
        ...
        <div className="relative group">
          <button onClick={handleLogout}
            className={[
              ...
              collapsed ? 'justify-center px-2' : 'gap-3 px-3',
              'text-ink-400 hover:bg-ink-700 hover:text-error',
            ].join(' ')}>
