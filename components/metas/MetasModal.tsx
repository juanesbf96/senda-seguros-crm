      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-slate-900">{meta ? 'Editar meta' : 'Nueva meta'}</h2>
          </div>
        ...
        <div className="px-6 py-5 space-y-4">
          {/* Nombre */}
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Nombre de la meta *</label>
          ...
          {/* Tipo */}
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1.5">Tipo de meta</label>
              ...
              {(Object.entries(TIPO_LABELS) as [TipoMeta, string][]).map(([k, label]) => (
                <button key={k} onClick={() => set('tipo', k)}
                  className={`text-left px-3 py-2 rounded-lg border text-xs font-medium transition-colors ${
                    form.tipo === k ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-600 hover:border-slate-300'
                  }`}>
                  {label}
                </button>
              ))}
              ...
              {(Object.entries(PERIODO_LABELS) as [PeriodoMeta, string][]).map(([k, label]) => (
                <button key={k} onClick={() => handlePeriodo(k)}
                  className={`px-3 py-1.5 rounded-lg border text-xs font-medium transition-colors ${
                    form.periodo === k ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-600 hover:border-slate-300'
                  }`}>
                  {label}
                </button>
              ))}
          ...
          <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
            <div>
              <p className="text-sm font-medium text-slate-700">Auto-calcular progreso</p>
              <p className="text-xs text-slate-500">El sistema actualiza el avance desde los datos del CRM</p>
            </div>
            <button onClick={() => set('auto_calcular', !form.auto_calcular)}
              className={`relative w-10 h-5 rounded-full transition-colors ${form.auto_calcular ? 'bg-emerald-500' : 'bg-slate-300'}`}>
          ...
          {!form.auto_calcular && (
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Progreso actual</label>
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-100">
          <button onClick={onClose} className="px-4 py-2 text-sm text-slate-600 hover:text-slate-800 rounded-lg border border-slate-200 hover:border-slate-300 transition-colors">
            Cancelar
          </button>
